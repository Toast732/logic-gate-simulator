#include "logic_pipe.hpp"

#ifndef LOGIC_PIPE_CPP
#define LOGIC_PIPE_CPP

#define LOGIC_PIPE_THICKNESS 0.15
#define LOGIC_PIPE_SIDES 15
#define PIPE_PARTICLE_SIDES 10
#define PIPE_PARTICLE_RADIUS 0.025

typedef ResourceManager<Ticker> Tickers;
typedef ObjectStorage<LogicPipe> LogicPipes;

inline float const HALF_PI = PI * 0.5;

inline std::vector<LogicPipe*> logic_pipes;

inline void tick_logic_pipe() {
	//TraceLog(LOG_INFO, "Ticking logic pipes");

	//std::vector<LogicPipe*> logic_pipes = LogicPipes::GetAll("logic_pipes");

	for (int i = 0; i < logic_pipes.size(); i++) {
		logic_pipes[i]->update();
	}
}
...
inline void setup_logic_pipe() {
	...
	std::shared_ptr<Ticker> ticker30 = Tickers::Get("ticker30");

	Tickers::Get("ticker30")->addTargetFunction(tick_logic_pipe);
}

		
inline PipeParticle::PipeParticle(Matrix start_pos, Matrix end_pos) {
...
inline void PipeParticle::update() {
...
inline SMatrix LogicPipe::calculateMatrix() {
...
inline LogicPipe::LogicPipe(LogicIO* source, LogicIO* target) {
	this->source = source;
	this->target = target;

	this->instance_ids.push_back(MeshInstances::Get("logic_pipe")->addInstance(this->calculateMatrix()));

	logic_pipes.push_back(this);
}
inline void LogicPipe::update() {
